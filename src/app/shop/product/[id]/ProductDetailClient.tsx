...
import { SelectedAttributes, ProductVariantDTO, ProductDTO } from "@/types/shop";
 ...
 * Handles interactive features: cart, quantity selection, variants
 * Receives initial data from server for hydration
 */
export default function ProductDetailClient({ productId, initialProduct }: ProductDetailClientProps) {
    // Use initialProduct as initial data for TanStack Query (hydration)
    ...
    // Show loading only if no initial data was provided
    if (isLoading && !initialProduct) return <div className="container py-12"><LoadingSkeleton /></div>;
    if (isError || !product) return (
        <div className="container py-20 text-center">
            <h1 className="text-2xl font-bold">المنتج غير موجود</h1>
            <p className="text-zinc-500 mt-2">عذراً، المنتج الذي تبحث عنه غير متوفر.</p>
        </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 pb-20">
            ...
            <div className="space-y-8 lg:sticky lg:top-24 h-fit">
                ...
                <div className="prose prose-zinc max-w-none">
                    <h2 className="text-lg font-semibold text-zinc-900 mb-2">تفاصيل المنتج</h2>
