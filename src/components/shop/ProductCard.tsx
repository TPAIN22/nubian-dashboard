...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/shop/product/${product._id}`} className="group relative block bg-white rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-zinc-200/50 hover:-translate-y-1">
      
      {/* Image Container */}
      <div className="relative aspect-[4/5] overflow-hidden bg-zinc-100/50">
        {product.images?.[0] ? (
          <Image
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 z-10">
          {hasDiscount && (
             <Badge className="rounded-full shadow-sm bg-red-500 hover:bg-red-600 text-white border-none px-3 py-1 text-xs">
               -{priceData.discount?.percentage}%
             </Badge>
          )}
          {isOutOfStock && (
             <Badge variant="secondary" className="rounded-full bg-zinc-900 text-white backdrop-blur-md px-3 font-normal">
               نفذت الكمية
             </Badge>
          )}
        </div>
      ...
      <div className="p-5">
        <div className="flex justify-between items-start gap-2 mb-2">
           <h3 className="font-semibold text-zinc-900 line-clamp-1 text-base group-hover:text-amber-700 transition-colors">
              {product.name}
           </h3>
        ...
        <p className="text-sm text-zinc-500 line-clamp-1 mb-4 h-5 font-light">{product.description}</p>
        
        <div className="flex items-center justify-between mt-auto">
           <div className="flex flex-col">
             <span className="font-bold text-lg text-zinc-900 font-numeric">
               {priceData.requiresSelection && <span className="text-xs font-normal text-zinc-400 ml-1">تبدأ من</span>}
               {priceData.final.toLocaleString()} <span className="text-xs font-medium text-zinc-400">ج.س</span>
             </span>
             {hasDiscount && (
           ...
           <Button 
              size="icon" 
              variant="secondary" 
              className="h-10 w-10 rounded-full md:hidden bg-zinc-100 text-zinc-900"
              onClick={handleQuickAdd}
