...
import { applyPatch, normalizeStateMap } from './state'
...
/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Two things live here: the merchant's progress through every tour (server-
   owned, cached locally) and the small amount of real evidence a tour needs to
   stop being generic — how many products they have, how many orders, whether
   the store profile is filled in.

   The evidence queries are gated on a tour actually being live AND on that tour
   needing them. The console shell mounts on every merchant route, and the
   codebase already made the call that a full catalogue fetch is too expensive
   to run on the settings page just to compute a number. So nothing here fires
   for a merchant who finished their tours, or while the add-a-product tour —
   which reads none of it — is the one on screen.
   ========================================================================== */
...
export type OnboardingStateQuery = {
  /** Every tour the merchant has touched, keyed by tour id. */
...
export function useOnboardingState(): OnboardingStateQuery {
  ...
  const query = useQuery<OnboardingStateMap>({
    queryKey: onboardingKeys.state(userId),
    queryFn: async () => normalizeStateMap(unwrap(await merchantRequest<unknown>(ENDPOINT))),
  ...
  if (!authLoaded || !userId) return { tours: null, loaded: false, degraded: false }

  if (query.isSuccess) {
    return { tours: query.data, loaded: true, degraded: false }
  }

  if (query.isError) {
    // A cached answer is better than no answer: it is right for this device and
    // it is the only thing standing between a merchant who already finished a
    // tour and seeing it again because the API blipped.
    return { tours: snapshot, loaded: snapshot !== null, degraded: true }
  }

  return { tours: null, loaded: false, degraded: false }
}

/**
 * Persists a partial update to one tour.
 *
...
export function useSaveOnboarding() {
  ...
  return useMutation({
    ...
    onMutate: async (patch) => {
      ...
      const previous = qc.getQueryData<OnboardingStateMap>(key)
    ...
    onSuccess: (body) => {
      const server = normalizeStateMap(unwrap(body))
 ...
 * the products page filters client-side. A tour only needs to know whether the
 * number is zero, so it asks for a single row and reads the pagination total —
 * one request, one document, on a shell that mounts everywhere.
 */
...
/**
 * What a tour knows about this merchant's actual situation.
 *
...
export function useMerchantContext(enabled: boolean): MerchantContext {
  ...
  return React.useMemo(() => {
    ...
    // Settled, not successful: a failed evidence query must not hold a tour
