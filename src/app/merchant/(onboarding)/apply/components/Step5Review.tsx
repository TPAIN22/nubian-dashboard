  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        {
          label: 'نوع التسجيل',
          value: data.merchantType === 'business' ? 'عمل مسجل (شركة/مؤسسة)' : 'فرد / عمل منزلي',
        },
    ...
    },
    {
      ...
      items: [{ label: 'الآيبان', value: data.iban }],
    },
    {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-700 mb-6">
          يرجى مراجعة بياناتك والتأكد من صحتها قبل إرسال الطلب إلى فريقنا.
        </p>
      </div>
          ...
          <div key={idx} className="bg-card text-card-foreground border rounded-xl overflow-hidden">
            ...
            <div className="p-4 bg-transparent space-y-3">
              {section.items.map((item, i) => (
                <div
                  key={i}
                  className="flex flex-col sm:flex-row sm:justify-between border-b pb-2 last:border-0 last:pb-0"
                >
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-sm font-medium leading-relaxed">
              ...
              و{' '}
              <a href="/privacy" target="_blank" className="text-primary hover:underline font-bold">
                سياسة الخصوصية
              </a>{' '}
              الخاصة بالمنصة.
            </span>
            <span className="text-xs text-muted-foreground">
              ستتم مراجعة طلبك يدوياً قبل الموافقة عليه.
            </span>
          </div>
        </label>
        {errors.agreedToTerms && (
          <p className="text-sm text-red-500 mt-2 pr-7">{errors.agreedToTerms.message}</p>
        )}
      </div>
    </div>
  )
