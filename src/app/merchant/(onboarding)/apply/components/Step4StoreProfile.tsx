...
export default function Step4StoreProfile() {
  const {
    ...
    watch,
    setValue,
  } = useFormContext<MerchantRegistrationData>()
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          تفاصيل حول ما تبيعه، وهي المعلومات التي ستظهر للعملاء.
        </p>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div>
          <Label className="font-semibold block mb-2">
            شعار المتجر <span className="text-red-500">*</span>
          </Label>
          <input
            type="file"
            accept="image/*"
            ...
            {isUploading ? (
              ...
              <>
                <div className="relative group">
                  <img
                    src={logoUrl}
                    alt="Store Logo"
                    className="w-16 h-16 object-cover rounded-lg shadow-sm border mb-2"
                  />
                  <button
                    type="button"
                    onClick={(e) => {
                ...
                <p className="text-sm text-green-700 font-medium flex items-center gap-1">
                  <Check className="w-4 h-4" /> تم رفع الشعار
                </p>
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  PNG أو JPG أو HEIC — يتم ضغط الصورة تلقائياً
                </p>
              </>
            )}
          </div>
        ...
        <div className="bg-muted/30 border border-border p-4 rounded-xl">
          <Label className="font-semibold block mb-1">
            عينات من منتجاتك <span className="text-red-500">*</span>
          </Label>
          <p className="text-xs text-muted-foreground mb-4">
            يرجى رفع 3 صور على الأقل لمنتجاتك للمراجعة.
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            {productSamples.map((url: string, index: number) => (
              <div
                key={index}
                className="relative group aspect-square border-2 border-border rounded-lg overflow-hidden bg-background"
              >
          ...
          {errors.productSamples && (
            <p className="text-sm text-red-500 mt-1">{errors.productSamples.message as string}</p>
          )}
        </div>
        ...
        <div className="bg-muted/30 border border-border p-4 rounded-xl">
          <Label className="font-semibold block mb-2">
            فئات المتجر <span className="text-red-500">*</span>
          </Label>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {categories.map((c) => (
              <span
                key={c}
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-primary text-primary-foreground"
              >
                {c}
                <button
                  type="button"
          ...
          {suggestedCategories.length > 0 && (
            <div className="mb-3 pt-2 border-t border-gray-200">
              <p className="text-xs text-blue-600 font-medium mb-1.5">
                اقتراحات الذكاء الاصطناعي (اضغط للإضافة):
              </p>
              <div className="flex flex-wrap gap-2">
                {suggestedCategories
                  .filter((sc) => !categories.includes(sc))
                  .map((sc) => (
                    <button
                      type="button"
                      key={sc}
          ...
          <div className="flex gap-2">
            ...
            />
            <Button type="button" variant="secondary" onClick={() => addCategory(categoryInput)}>
              إضافة
            </Button>
          </div>
          {errors.categories && (
