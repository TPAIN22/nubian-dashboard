...
export default function OnboardingWizard() {
  ...
  const onSubmit = async (data: MerchantRegistrationData) => {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        const fallback = pickErrorMessage(
          code,
          'لا يمكن إعادة تقديم الطلب في حالته الحالية.',
  ...
  if (isSuccess) {
    return (
      <div className="animate-in fade-in zoom-in flex flex-col items-center justify-center py-10 text-center duration-500">
        <div className="mb-5 grid size-12 place-items-center rounded-full border border-tone-success-border bg-tone-success-bg">
          <CheckCircle2 className="size-6 text-tone-success-fg" />
        </div>
        <h2 className="text-[17px] font-semibold tracking-[-0.011em] text-foreground">
          تم استلام طلبك
        </h2>
        <p className="mx-auto mt-2 max-w-md text-[13px] leading-6 text-text-muted">
          سيراجع فريقنا الطلب ويرد عليك خلال 24 إلى 48 ساعة. يمكنك متابعة الحالة في أي وقت من صفحة
          حالة الطلب.
        </p>
        <div className="mt-6 flex items-center gap-2">
          <Button onClick={() => router.push('/merchant/pending')}>متابعة حالة الطلب</Button>
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 3:
        return <Step3Financials />
      case 4:
        return <Step4StoreProfile />
      case 5:
        return <Step5Review />
      default:
        return null
  ...
  return (
    <FormProvider {...methods}>
      <form
        onSubmit={(e) => Object.keys(methods.formState.errors).length > 0 && e.preventDefault()}
      >
        {/*
          Stepper. The rail is RTL-native: it is pinned with logical inset-inline
          properties, so the fill grows from the start edge in both directions.
          The old one used `left-0` and grew the wrong way in Arabic.
        */}
        <div className="mb-10">
          <div className="relative flex items-center justify-between">
            <div className="absolute inset-x-0 top-1/2 z-0 h-0.5 -translate-y-1/2 rounded-full bg-border" />
            <div
              className="absolute top-1/2 z-0 h-0.5 -translate-y-1/2 rounded-full bg-foreground transition-all duration-300 ltr:left-0 rtl:right-0"
            ...
            {steps.map((step) => (
              <div key={step.id} className="relative z-10 flex flex-col items-center">
                <div
                  className={`grid size-7 place-items-center rounded-full text-[12px] font-medium transition-colors duration-300 ${
                    currentStep >= step.id
                      ? 'bg-primary text-primary-foreground'
                      : 'border border-border bg-background text-text-faint'
                  }`}
                >
                  {currentStep > step.id ? <CheckCircle2 className="size-4" /> : step.id}
                </div>
                <span
                  className={`absolute -bottom-6 text-[11px] font-medium whitespace-nowrap ${
                    currentStep >= step.id ? 'text-foreground' : 'text-text-faint'
                  }`}
                >
                  {step.title}
                </span>
        ...
        <div className="animate-in slide-in-from-right-4 fade-in mt-12 mb-8 min-h-[300px] duration-300">
          {renderStep()}
        </div>
