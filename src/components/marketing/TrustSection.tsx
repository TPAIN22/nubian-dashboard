  ...
  return (
    <section className="py-24 bg-white">
      <div className="container max-w-7xl mx-auto px-6">
        <div className="p-12 md:p-20 bg-zinc-950 rounded-[2rem] text-white flex flex-col md:flex-row-reverse items-center justify-between gap-12 overflow-hidden relative">
          ...
          <div className="grid grid-cols-2 gap-x-12 gap-y-12 relative z-10 text-right">
            {stats.map((stat, idx) => (
              <div key={idx} className="flex flex-col gap-2">
                ...
                <div className="text-sm font-bold tracking-widest text-zinc-500 uppercase">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full opacity-10 pointer-events-none">
             <div className="w-[150%] h-[150%] border-2 border-white rounded-full translate-x-1/2" />
          </div>
        </div>
      </div>
    </section>
